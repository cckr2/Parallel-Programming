...
int main(int argc, char **argv){
	...
	val2 = (float*)malloc(sizeof(float)*maxnp*niter);
	...
	MPI_Reduce(val,val2,maxnp*niter,MPI_FLOAT,MPI_SUM,ROOT,MPI_COMM_WORLD);
	MPI_Bcast(val2,maxnp*niter,MPI_FLOAT,ROOT,MPI_COMM_WORLD);
	free(val);
	/*
	np = niter*maxnp;
	for(i=0;i<np;i++){
		j = val2[i]/step;
		*(lval[j]+nlval[j]) = val2[i];
		nlval[j]++;
	}*/

	for(i=(maxnp*nRank);i<maxnp*(nRank+1);i++){
		j = val2[i]/step;
		*(lval[j]+nlval[j]) = val2[i];
		nlval[j]++;
	}
	MPI_Reduce(nlval,nlval2,niter,MPI_FLOAT,MPI_SUM,ROOT,MPI_COMM_WORLD);
	

	if(nRank==0){
		for(i=0;i<niter;i++){
			printf("p%d has %ld members ::: %g %g\n", (int)i,nlval2[i], step*i, step*(+1));
		}
	}
	time2 = gettime();
