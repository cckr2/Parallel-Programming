...
void interface_flux(double *qq, double *fstar, double *ib, double speed, int nRank, int nProcs);
void rhs(double *qq, double *rr, double *dv, double *df, double *ib, double speed, int nRank, int nProcs);
...
int main(int argc, char **argv){
	...
	long nstep;
	double *dx, *mesh; 
	double *smat, *xx, *qq, *qtemp,*qtemp2, *k1, *k2, *k3, *k4, *minv_vec, *mmat, *dv, 
		   *mf, *ib, *df, *fstar;
	...
	// ci=0;ii<ne*np;ii++)
	// (1d index structure array = [ee*np*np + jj*np + ii]
	...
	qtemp2	= (double*)malloc(ne*np*sizeof(double));    // [ee ne, ii np]	
	...
	while(rtime < tend){
		...
		rhs(qq,	   k1, dv, df, ib, speed, nRank, nProcs);

		for(ii=0;ii<ne*np;ii++)
			qtemp[ii] = qq[ii]+0.5*dt*k1[ii];
		rhs(qtemp, k2, dv, df, ib, speed, nRank, nProcs);

		for(ii=0;ii<ne*np;ii++)
			qtemp[ii] = qq[ii]+0.5*dt*k2[ii];
		rhs(qtemp, k3, dv, df, ib, speed, nRank, nProcs);
	
		for(ii=0;ii<ne*np;ii++)
			qtemp[ii] = qq[ii]+dt*k3[ii];
		rhs(qtemp, k4, dv, df, ib, speed, nRank, nProcs);
	...
	for(int i=0;i<ne;i++) {
		printf("dt = %f k1 = %f k2 = %f k3 = %f k4 = %f qq = %f \n",dt,k1[ii],k2[ii],k3[ii],k4[ii], qq[i]);
	}
	free(roots);   
	free(weights); 
	free(ll);      
...
void interface_flux(double *qq, double *fstar, double *ib, double speed, int nRank, int nProcs){
	...
	double tmp[2*ne];
	MPI_Reduce(&qb,&tmp, 2*ne, MPI_DOUBLE, MPI_SUM, ROOT, MPI_COMM_WORLD); 
	...
	MPI_Bcast(&tmp, 2*ne, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);
	ii = 0; // calculating numerical flux (fstar) with periodic boundary condition
	fstar[ii] = -((tmp[ne+ne-1]+tmp[ii])/2*speed + fabs(speed)*(tmp[ne+ne-1]-tmp[ii])/2);
	fstar[ne+ne-1] = -fstar[0];
	for(ii=1;ii<ne;ii++){
		fstar[ii] = -((tmp[ne+ii-1]+tmp[ii])/2*speed + fabs(speed)*(tmp[ne+ii-1]-tmp[ii])/2);
...
void rhs(double *qq, double *rr, double *dv, double *df, double *ib, double speed, int nRank, int nProcs){
	int ii, jj, ee, idx;
	double fstar[2*ne], term, *temp,*temp2;
	temp =  (double*)malloc(ne*np*sizeof(double));
	...
	for(ee=jsta;ee<jend;ee++){
		for(ii=0;ii<np;ii++){
			for(jj=0; jj<np; jj++){
				temp[ee*np+ii] += dv[ee*np*np+jj*np+ii]*speed*qq[ee*np+jj];
			}
		}
	}


	interface_flux(qq, fstar, ib, speed, nRank, nProcs);
	for(ee=jsta;ee<jend;ee++){
		for(ii=0; ii<np; ii++){
			for(jj=0;jj<2;jj++){
				temp[ee*np+ii] -= df[ee*(np*2)+jj*np+ii]*fstar[ee+jj*ne];
			}
		}
	}

	temp2 =  (double*)malloc(ne*np*sizeof(double));
 	MPI_Reduce(temp,temp2,np*ne, MPI_DOUBLE, MPI_SUM, ROOT, MPI_COMM_WORLD);
	MPI_Bcast(temp2,np*ne, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);
	rr =temp2;
	free(temp);
}
