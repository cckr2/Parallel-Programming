...
int main(int argc, char **argv){
	...
	while(rtime < tend){
		...
		for(ii=jsta;ii<jend;ii++)
			qtemp2[ii] = qq[ii]+0.5*dt*k1[ii];
	
		MPI_Reduce(qtemp2,qtemp,np*ne, MPI_DOUBLE, MPI_SUM, ROOT, MPI_COMM_WORLD); 
		MPI_Bcast(qtemp,np*ne, MPI_DOUBLE, ROOT, MPI_COMM_WORLD);
