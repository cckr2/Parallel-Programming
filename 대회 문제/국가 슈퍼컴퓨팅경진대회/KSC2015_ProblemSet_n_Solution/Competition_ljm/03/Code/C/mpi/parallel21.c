...
void domaindecomp(float **ibase, size_t *mmen, float valmin, float valmax, MPI_Comm Comm) {
	...
	MPI_Reduce(number,receive,32,MPI_LONG,MPI_SUM,0,MPI_COMM_WORLD);
	MPI_Bcast(receive,32,MPI_LONG, 0, MPI_COMM_WORLD);
	float *res;
	res= (float*)malloc(sizeof(float*)*receive[nRank]);
	for(i=0;i<number[nRank];i++){
		...
		if(nRank==i) {
			for(j=0;j<32;j++) {
				if(nRank==j) 
					...
					MPI_Irecv(&recvnum, 1, MPI_LONG,  j, 20, MPI_COMM_WORLD, &req2);
			...
			MPI_Isend(randnum[i],number[i], MPI_LONG, i, 10, MPI_COMM_WORLD, &req3);
			MPI_Wait(&req3, &status);
		}
	}
	*mmen =receive[nRank];
	**ibase = *res;
	printf("count : %lf receive[nRank] : %lf",count,receive[nRank]);
}
